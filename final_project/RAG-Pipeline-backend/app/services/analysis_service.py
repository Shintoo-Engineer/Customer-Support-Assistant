...
INTENT_KEYWORDS: Dict[CustomerIntent, List[str]] = {
    CustomerIntent.REFUND: [
        ...
        "double charge", "unexpected charge", "charged", "charge", "get my money back",
        "want a refund", "need a refund", "request a refund", "refund for"
    ],
    ...
    CustomerIntent.DELIVERY_ISSUE: [
        ...
        "delayed delivery", "why hasn't my order arrived", "completely smashed and damaged",
        "smashed and damaged", "smashed", "delivery arrived"
    ],
    CustomerIntent.PAYMENT_ISSUE: [
        "card declined", "payment failed", "credit card", "debit card",
        "checkout", "transaction", "3ds", "3d-secure", "declined", "declining", "billing failed",
        "err_payment", "payment link", "pay link", "payment error", "timing out", "payment",
        "keeps declining", "bypass this error", "funds are sufficient", "plenty of funds",
        "enough funds", "keeps failing", "payment keeps", "card keeps"
    ],
    CustomerIntent.ACCOUNT_ISSUE: [
        ...
        "reset link", "security code", "magic link", "credentials", "can't log", "cannot log"
    ],
    CustomerIntent.COMPLAINT: [
        ...
        "unhappy", "dissatisfied", "very unhappy", "extremely unhappy", "useless"
    ],
    CustomerIntent.RETURN_EXCHANGE: [
        "exchange", "swap", "wrong size", "replace", "replacement", "return item",
        "send back", "defective item", "broken item", "return", "damaged product",
        "damaged item", "damaged", "defective", "broken"
    ],
...
EMOTION_KEYWORDS: Dict[CustomerEmotion, List[str]] = {
    CustomerEmotion.ANGRY: [
        "angry", "extremely angry", "very angry", "so angry", "furious", "pissed", "unacceptable",
        "fix this right now", "fix this now", "get me your manager", "demand", "ridiculous",
        "not paying for this", "disgusted", "lawyer", "sue", "manager immediately",
        "completely unacceptable", "right now", "stop charging", "terrible", "horrible", "awful",
        "worst", "incompetent", "formal complaint", "file a complaint", "give me my refund",
        "charged me twice", "you people are useless", "useless", "pathetic", "scam", "rip off",
        "completely smashed", "smashed and damaged", "smashed"
    ],
    CustomerEmotion.FRUSTRATED: [
        "frustrated", "really frustrated", "very frustrated", "extremely frustrated", "frustrating",
        "already told you", "taking too long", "wasting my time", "tired of waiting",
        "explained this twice", "contacted you three times", "still waiting", "still no update",
        "no straight answer", "unhelpful", "taking way longer", "don't have time for this",
        "already", "still haven't", "still hasn't", "hasn't arrived", "not arrived", "why hasn't",
        "urgent", "too long", "waiting for hours", "refund immediately", "disappointed", "dissatisfied",
        "third time", "second time", "fourth time", "nobody is resolving", "nobody has helped",
        "nobody helps", "waiting for days", "been waiting for days", "keeps declining",
        "bypass this error", "keeps failing", "declined again", "fails again", "third time i've complained",
        "complained multiple times", "waste of time"
    ],
    ...
    CustomerEmotion.SATISFIED: [
        ...
        "thank you for your assistance", "thank you for your help", "went through successfully",
        "went through", "link worked", "now upgraded", "transaction went through"
    ],
    CustomerEmotion.HAPPY: [
        ...
        "happy to", "thank you very much", "so grateful"
    ],
}
...
def get_conversation_context(session_id: int, db: DBSession) -> Optional[Dict[str, Any]]:
    ...
    return {
        ...
        "initial_emotion": conversation_row.sentiment if conversation_row else None,
    }
...
def classify_emotion_deterministic(
    text: str,
    default_emotion: Optional[CustomerEmotion] = None
) -> CustomerEmotion:
    ...
    if default_emotion and isinstance(default_emotion, CustomerEmotion):
...
def determine_satisfaction_trend(
    ...
    has_prior_negative = any(
        classify_sentiment_deterministic(p, classify_emotion_deterministic(p)) == CustomerSentiment.NEGATIVE
        or classify_emotion_deterministic(p) in [CustomerEmotion.ANGRY, CustomerEmotion.FRUSTRATED]
        for p in prior_customer_msgs
    )

    if diff <= -2 or (prev_sentiment == CustomerSentiment.NEGATIVE and current_sentiment in [CustomerSentiment.NEUTRAL, CustomerSentiment.POSITIVE]) or (current_sentiment == CustomerSentiment.POSITIVE and has_prior_negative):
...
def analyze_customer_message(
    ...
    context_initial_emotion = context.get("initial_emotion")
    default_emo: Optional[CustomerEmotion] = None
    if context_initial_emotion:
        raw_emo_norm = str(context_initial_emotion).lower().strip()
        if raw_emo_norm in [e.value for e in CustomerEmotion]:
            default_emo = CustomerEmotion(raw_emo_norm)
        elif raw_emo_norm in ["negative", "unresolved"]:
            default_emo = CustomerEmotion.FRUSTRATED

    if default_emo is None and dialogue_history:
        prior_msgs = [m.get("message_text", "") for m in dialogue_history if m.get("sender_type") == "Customer"]
        if prior_msgs:
            last_emo = classify_emotion_deterministic(prior_msgs[-1])
            if last_emo in [CustomerEmotion.FRUSTRATED, CustomerEmotion.ANGRY, CustomerEmotion.WORRIED, CustomerEmotion.CONFUSED]:
                lower_curr = customer_message.lower()
                if not any(g in lower_curr for g in ["thank you", "thanks", "resolved", "fixed", "all good", "sorted", "great", "helpful"]):
                    default_emo = last_emo
    ...
    if not emotion_val:
        emotion_val = classify_emotion_deterministic(customer_message, default_emotion=default_emo)
    if not sentiment_val:
        sentiment_val = classify_sentiment_deterministic(customer_message, emotion_val)

    # 3b. Post-LLM Emotion Consistency Override
    # If the LLM returned a negative emotion but the message contains strong
    # positive/resolution signals, override with deterministic classification.
    # This prevents contradictory outputs (e.g. emotion=frustrated but frustration=1/10).
    if emotion_val in [CustomerEmotion.FRUSTRATED, CustomerEmotion.ANGRY, CustomerEmotion.WORRIED]:
        lower_msg = customer_message.lower()
        positive_resolution_signals = [
            "thank you", "thanks", "thank you for your assistance",
            "thank you for your help", "went through successfully",
            "transaction went through", "resolved", "all good", "all sorted",
            "working now", "fixed now", "appreciate", "link worked",
            "successful", "that works", "sorted out", "all set",
            "glad", "great", "wonderful", "amazing",
        ]
        negative_signals = [
            "unacceptable", "ridiculous", "angry", "furious", "demand",
            "manager", "supervisor", "complaint", "useless", "terrible",
            "horrible", "awful", "failing", "keeps failing", "can't help",
            "nothing we can do", "deal with it", "refuse", "waiting",
            "still haven't", "still hasn't", "disappointed", "dissatisfied",
        ]
        has_positive = any(sig in lower_msg for sig in positive_resolution_signals)
        has_negative = any(sig in lower_msg for sig in negative_signals)
        if has_positive and not has_negative:
            det_emotion = classify_emotion_deterministic(customer_message, default_emotion=default_emo)
            if det_emotion in [CustomerEmotion.SATISFIED, CustomerEmotion.HAPPY]:
                logger.info(
                    "emotion_consistency_override session_id=%s llm_emotion=%s -> deterministic_emotion=%s",
                    session_id, emotion_val.value, det_emotion.value
                )
                emotion_val = det_emotion
                sentiment_val = classify_sentiment_deterministic(customer_message, emotion_val)
